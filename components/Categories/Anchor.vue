    ...
    <div class="py-2.5 w-full">
      <ul class="overflow-auto">
        <li class="overflow-hidden  h-14 rounded-l-md" v-for="(items,index) in category" :key="items.id ">
          <div :class="{active:active == items.id }" @click="goId(items.id)" class="text-white px-5 block text-lg leading-[3.5rem] h-full opacity-70 bg-gradient-to-r hover:from-black hover:to-stone-900">{{items.name}}</div>
        </li>
      </ul>
    </div>
  </div>
